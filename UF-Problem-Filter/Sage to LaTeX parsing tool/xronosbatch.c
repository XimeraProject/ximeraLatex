...
/* set default log file name */
	...
	p = DL_GetIniValue( "IndividualLogFiles", NULL );
	if ( p != NULL )
	{
		DL_STRUPR( (unsigned char *) p );
		if ( strcmp( p, "Y" ) == 0 || strcmp( p, "YES" ) == 0 )
			IndividualLogFiles = TRUE;
	}
	
	p = DL_GetIniValue( "CleanupPDFWorkFiles", NULL );
	if ( p != NULL )
	{
		DL_STRUPR( (unsigned char *) p );
		if ( strcmp( p, "N" ) == 0 || strcmp( p, "NO" ) == 0 )
			CleanupPDFWorkFiles = TRUE;
	}
	
	p = DL_GetIniValue( "CleanupSageWorkFiles", NULL );
	if ( p != NULL )
	{
		DL_STRUPR( (unsigned char *) p );
		if ( strcmp( p, "N" ) == 0 || strcmp( p, "NO" ) == 0 )
			CleanupSageWorkFiles = TRUE;
	}
	
	p = DL_GetIniValue( "CleanupPDFAndSageFiles", NULL );
	if ( p != NULL )
	{
		DL_STRUPR( (unsigned char *) p );
		if ( strcmp( p, "Y" ) == 0 || strcmp( p, "YES" ) == 0 )
			CleanupPDFAndSageFiles = TRUE;
	}
