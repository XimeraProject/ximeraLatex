	...
	*p = 0;
	...
	if ( ctxt != NULL ) /* got it */
	{
		...
		if ( p != NULL && *p != 0 )
		{
			...
			TrimQuotesFromString( ctxt->OptionValue );
...
#if 0
			if ( QuestionsOnly )
			{ /* if questions only, do we have a xronosfmt_option line in the file */
				ctxt = FindXronosfmtOption( "Path" );
				if ( ctxt != NULL && strlen( ctxt->OptionValue ) > 0 ) /* got a valid one */
				{
					/* assign the path from the option */
					strcpy( path, ctxt->OptionValue );
