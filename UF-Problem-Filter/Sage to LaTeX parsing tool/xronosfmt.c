...
#if 0
	p = DL_GetIniValue( "XF_OutputDir", NULL );
	if ( p != NULL )
	{
...
if ( strlen(xronosfmt_outputpath) > 0 )
{
	...
	p = DL_GetIniValue( "XF_OutputFileHeader", NULL );
	if ( p != NULL )
	{
	XTL_TrimQuotesInPlace( p );
	...
	p = DL_GetIniValue( "XF_OutputFileFooter", NULL );
	if ( p != NULL )
	{
	XTL_TrimQuotesInPlace( p );
	...
	p = DL_GetIniValue( "XF_InputFileHeader", NULL );
	if ( p != NULL )
	{
	XTL_TrimQuotesInPlace( p );
